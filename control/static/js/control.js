document.addEventListener("DOMContentLoaded", function () {
	...
	function updateCounts() {
		const studentsCount = document.querySelectorAll(
			".column:nth-child(2) .user",
		).length;
		const requestsCount = document.querySelectorAll(
			".column:nth-child(3) .user",
		).length;

		document.querySelector(".column:nth-child(2) h3").textContent =
			`Учасники (${studentsCount})`;
		document.querySelector(".column:nth-child(3) h3").textContent =
			`Запити (${requestsCount})`;
	}
	...
	document.addEventListener("click", function (e) {
		if (e.target.closest(".btn-accept")) {
			...
			fetch(`/accept_student/${studentId}`, {
				...
				.then((data) => {
					if (data.success) {
						const studentsColumn = document.querySelector(
							".column:nth-child(2)",
						);
						const userClone = userDiv.cloneNode(true);

						const acceptBtn = userClone

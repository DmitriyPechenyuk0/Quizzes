...
def is_correctt(user_text: str, correct_raw: str, type: str) -> bool:
    if type in ('fform', 'letters'):
        return normalize(user_text) == normalize(correct_raw or "")
    if type == 'select':
        # correct_answer для select зберігає ПЕРШИЙ варіант як правильний
        # формат: "Правильний текст:мітка|Варіант2:мітка2|..."
        correct_text = (correct_raw or "").split('|')[0].split(':')[0].strip()
        return user_text.strip() == correct_text
    return False
...
def current_question(session: QuizSession):
    ...
    return Question.query.filter_by(
        ...
        order_index=session.current_order
    ).first()
...
def broadcast_state(code: str):
    ...
    data = {
        ...
        "current_order": sessio.current_order,
        "quiz_name":     quiz.name    if quiz else "—",
        "quiz_code":     code,
        "quiz_subject":  quiz.subject if quiz else "—",
        "quiz_owner":    serialize_user(owner_user),   # ← FIX
    }
...
def on_join(data):
    ...
    if as_host and getattr(current_user, "is_teacher", False):
        teacher = SessionParticipant.query.filter_by(
            session_id=sess.id, user_id=current_user.id
        )
        if teacher:
            teacher.delete()
        db.session.commit()
    ...
    p = SessionParticipant.query.filter_by(
        session_id=sess.id, user_id=current_user.id
    ).first()
    ...
    if not p:
        ...
        try:
            ...
            db.session.rollback()
            return emit("error", {"message": "DB error on join"})
    ...
    if is_new_participant:
        emit(
            ...
            {"nickname": display_name, "id": display_id, "sess_status": sess.status},
            to=code
        )
...
def on_teacher_start(data):
    ...
    first_q = (
        Question.query
        .filter_by(quiz_id=sess.quiz_id)
        .order_by(Question.order_index.asc())
        .first()
    )
    if not first_q:
...
def check_answer(data):
    ...
    answered = SessionAnswer.query.filter_by(
        session_id=session.id, question_id=cur_quest.id
    ).count()
...
def on_answer(data):
    ...
    exists = SessionAnswer.query.filter_by(
    ...
    for participant in all_participants:
        ...
        participants_status.append({
            ...
            "is_correct": next(
                (ans.is_correct for ans in answered_users if ans.user_id == user.id),
                None
            ),
        })
...
def on_remove_user(data):
    ...
    try:
        ...
        emit("room:user_kicked", {
            "id": removed_id, "nickname": removed_nickname
        }, to=code)
        broadcast_state(code)
    except Exception as e:
        db.session.rollback()

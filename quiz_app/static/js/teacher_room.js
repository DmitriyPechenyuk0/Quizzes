(function () {
    ...
    function attachEvents() {
      ...
      socket.on("room:participants_update", (info) => {
        ...
        statusImg.className = "remove-user-button"; 
        statusImg.id = `rmusr_${info.id}`; 
        statusImg.src = 'http://127.0.0.1:5000/quiz/quiz_static/images/no_answer.svg';
        ...
        keepButton.className = "action-button keep-button";
        keepButton.textContent = "Залишити";
        
        let removeButton = document.createElement("button");
        removeButton.className = "action-button remove-button";
        removeButton.textContent = "Видалити";

        actionButtons.appendChild(keepButton);
        actionButtons.appendChild(removeButton);
        morediv.appendChild(actionButtons);
        ...
        removeButton.addEventListener('click', (e) => {
          e.stopPropagation();
          userRemover(info.id);
          socket.emit('rm_user_from_session', {code, user_id: info.id});
          updateUserCounter();
        });

        socket.on(`user_answer_status:${info.id}`, (status) => {
          statusImg.src = status.answered ? 
            'http://127.0.0.1:5000/quiz/quiz_static/images/yes_answer.svg' : 
            'http://127.0.0.1:5000/quiz/quiz_static/images/no_answer.svg';
        });
      });
    }
